    ...
    >
      ...
      <div className="relative bg-white rounded-2xl sm:rounded-3xl shadow-2xl shadow-indigo-500/10 w-full max-w-md max-h-[min(92dvh,90vh)] overflow-y-auto animate-slideInUp border border-slate-200/60">
        <div className="relative p-6 sm:p-8 border-b border-slate-100 bg-gradient-to-b from-slate-50/90 to-white">
          <button 
            onClick={onClose}
            className="absolute top-4 right-4 text-slate-400 hover:text-slate-700 hover:bg-slate-100 p-2 rounded-lg transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-indigo-500/30"
            aria-label="Close login modal"
          >
            <X className="w-5 h-5" />
          </button>
          <div className="flex flex-col items-center text-center gap-3 sm:gap-4 max-w-sm mx-auto px-8 sm:px-0">
            <NeelaLogo variant="full" size="md" showGlow={false} />
            <div className="space-y-1 w-full">
              <h3 id="login-modal-title" className="text-xl sm:text-2xl font-bold text-slate-900 tracking-tight">
