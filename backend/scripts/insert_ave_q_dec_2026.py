"""Insert Avenue Q Dec 2026 P&L with per-door income and unit-level expenses."""
import os
import sys
...
EFFECTIVE_INCOME = Decimal('4200.00')
...
def main():
    ...
    dec = result['monthly'][11]

    print('Avenue Q Dec 2026 inserted')
    print(f"  Dec cash flow:  income=${dec['income']:,.2f} expenses=${dec['expenses']:,.2f} net=${dec['net']:,.2f}")
    print(f"  NOI (Dec):      ${dec['net']:,.2f}")
