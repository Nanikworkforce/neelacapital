"""Insert Avenue Q Oct 2026 P&L with per-door income and unit-level expenses."""
import os
import sys
...
EFFECTIVE_INCOME = Decimal('4767.35')
...
def main():
    ...
    oct_ = result['monthly'][9]

    print('Avenue Q Oct 2026 inserted')
    print(f"  Oct cash flow:  income=${oct_['income']:,.2f} expenses=${oct_['expenses']:,.2f} net=${oct_['net']:,.2f}")
    print(f"  NOI (Oct):      ${oct_['net']:,.2f}")
