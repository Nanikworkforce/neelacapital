"""Insert Avenue Q Sep 2026 P&L with per-door income and unit-level expenses."""
import os
import sys
...
DOORS = [
    ...
    (2, Decimal('1225.89'), 'vacant'),
    (3, Decimal('1319.00'), 'occupied'),
...
EFFECTIVE_INCOME = Decimal('5394.89')
...
def main():
    ...
    sep = result['monthly'][8]

    print('Avenue Q Sep 2026 inserted')
    print(f"  Sep cash flow:  income=${sep['income']:,.2f} expenses=${sep['expenses']:,.2f} net=${sep['net']:,.2f}")
    print(f"  NOI (Sep):      ${sep['net']:,.2f}")
