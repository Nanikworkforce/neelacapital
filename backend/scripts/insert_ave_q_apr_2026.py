"""Insert Avenue Q Apr 2026 P&L with per-door income and unit-level expenses."""
import os
import sys
...
DOORS = [
    ...
    (2, Decimal('1722.00'), 'vacant'),
    (3, Decimal('368.80'), 'occupied'),
    (4, Decimal('1016.00'), 'occupied'),
]
...
EFFECTIVE_INCOME = Decimal('4866.80')
...
def main():
    ...
    print('Avenue Q Apr 2026 inserted')
    print(f"  Apr cash flow:  income=${apr['income']:,.2f} expenses=${apr['expenses']:,.2f} net=${apr['net']:,.2f}")
    print(f"  NOI (Apr):      ${apr['net']:,.2f}")
